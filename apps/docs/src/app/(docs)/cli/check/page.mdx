...
import { CHECK_LIST } from "@/lib/counts";
import {
  ASSERT_OUTPUT,
  ASSERT_RUN,
  CHECK_USAGE,
...
| `--token <value>` | An ownership token that must be published, compared byte for byte. |
| `--token-at <label>` | The name the token goes at, e.g. `_pg-challenge`. The apex by default. |
| `--cname <label=target>` | An alias that must point at a target. Repeatable: pass it once per alias. |
...
| `--only <kinds>` | Comma-separated subset of `{CHECK_LIST}`. Runs every check that has something to compare against when omitted, and refuses `ownership` or `cname` named without a value. |
