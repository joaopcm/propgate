...
export const EVALUATOR_ENTRIES = {
  ...
  delegation: {
    ...
    nonObvious:
      "every nameserver in the delegation is queried individually, because a lame delegation is a fact about one server, not about the zone — resolvers that happen to pick it get SERVFAIL while everyone else is fine.",
  },
  dkim: {
    code: DKIM_SAMPLE,
    nonObvious:
  ...
  dmarc: {
    code: DMARC_SAMPLE,
    nonObvious:
      "a record is valid at the exact name or, failing that, at the organizational domain — and an external rua= destination has to publish its own authorisation record back, or the reports it is sent are silently discarded.",
  },
  mx: {
    code: MX_SAMPLE,
    nonObvious:
...
- **A skipped check is not a passing check.** A profile that never asks about
  DKIM produces no DKIM outcome at all — not a green one. `sendingOnly`,
...
Rendered from `REQUIREMENT_TYPES`, the same record the API reference reads —
...
<div>
  {CHECK_KINDS.map((kind) => {
    ...
    return (
      <section className="my-8 border-white/5 border-t pt-6" key={kind}>
