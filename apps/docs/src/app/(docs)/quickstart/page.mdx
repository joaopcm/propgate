...
including the unflattering ones. The one exception is the signup exchange
below, which ends in a mailbox rather than on a terminal — its request and
response shapes come from the code, and it is marked where it appears.
...
**only when an observation actually differs** — that is the difference
between a $20 infrastructure bill and a $400 one, and it cannot be
retrofitted.
